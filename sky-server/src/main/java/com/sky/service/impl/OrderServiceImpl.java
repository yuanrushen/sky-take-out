...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    @Transactional
    public OrderSubmitVO submitOrdet(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        shoppingCartMapper.deleteshopping(userid);
...
//                .id(ordersDB.getId()).status(Orders.TO_BE_CONFIRMED)
//                .payStatus(Orders.PAID).checkoutTime(LocalDateTime.now())
//                .build();
...
//            log.info("申请退款：{}", refund);
    ...
    public void Cancel(OrdersCancelDTO ordersCancelDTO) {
        ...
        // 订单只有存在且状态为2（待接单）才可以拒单
        if (ordersDB == null || ordersDB.getStatus()>Orders.COMPLETED) {
            throw new OrderBusinessException(MessageConstant.ORDER_STATUS_ERROR);
        }
...
//            log.info("申请退款：{}", refund);
    ...
    @Override
    public List<Orders> processTimeOut(Integer status, LocalDateTime now) {
        return orderMapper.getBystatusAndOrderTime(status, now);
    }
